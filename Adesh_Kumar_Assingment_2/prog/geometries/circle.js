...
class Circle extends Geometry {
  ...
  generateCircleVertices(radius, segments, centerX, centerY) {
    ...
    for(var i = 0; i < segments*2; ++i){
      ...
      //outer point 1
      verts.push( radius * Math.cos(i * Math.PI / segments) + centerX, radius * Math.sin(i * Math.PI / segments) + centerY, 0 );
      //outer point 2
      verts.push( radius * Math.cos((i+1) * Math.PI / segments) + centerX, radius * Math.sin((i+1) * Math.PI / segments) + centerY, 0 );
    }
    
    super.vertices = verts;
